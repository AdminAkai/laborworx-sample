...
def import_data():
    ...
    with open(fname, newline='', encoding='utf-8-sig') as csvfile:
        ...
        for row in reader:
            ...
            if row['PhoneNumber'] and row['PhoneNumber'].strip():
                batch.append(row)
            ...
            for i in range(len(rows)):                
                batch.put_item(
                    Item={
                        ...
                        # 'AvailableToWork': bool(rows[i]['AvailableToWork']),
                        'trade': rows[i]['Trade'],
                        # 'YearsInTrade': int(rows[i]['YearsInTrade']),
                        'state': rows[i]['State'],
                        'completedDrugScreen': bool(rows[i]['CompletedDrugScreen']),
                        'isW2': bool(rows[i]['IsW2']),
                        'is1099': bool(rows[i]['Is1099']),
                        'newLocation': rows[i]['NewLocation']                        
                    }
                )
